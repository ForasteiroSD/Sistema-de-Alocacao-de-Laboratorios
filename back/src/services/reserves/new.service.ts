...
import { createReserveEmailText } from 'src/utils/sendEmail.js';
import { DailyReserve, PersonalizedReserves, ReserveInsert, UniqueReserve, WeeklyReserves } from 'src/utils/validation/reserve.schema.js';
...
export async function newReserve(req: Request, res: Response) {
    ...
    const userId: string = (req as any).userData.id;
    ...
    try {
        ...
        if (tipo === 'Semanal') {
            ...
            const dias_res_semana = horarios.map((horario: any) => {
                return {
