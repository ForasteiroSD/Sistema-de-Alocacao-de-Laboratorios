...
import { DailyReserve, idSchema, PersonalizedReserves, ReserveInsert, ReserveRemove, Reserves, ReservesRespLab, ReservesUser, UniqueReserve, WeeklyReserves } from '../schemas.js';
...
router.post('/reserva', async (req, res) => {
    ...
    if (tipo === "Personalizada") {
        ...
        if (tipo === "Única")
            newParse = UniqueReserve.safeParse(req.body);
        else if (tipo === "Diária")
            newParse = DailyReserve.safeParse(req.body);
        else
            newParse = WeeklyReserves.safeParse(req.body);
        if (!newParse.success) {
            return res.status(422).send({
                message: "Dados inválidos",
                errors: newParse.error.flatten()
...
router.post('/reservas/lab', async (req, res) => {
...
router.post('/reservas/user', async (req, res) => {
...
router.get('/reservas', adm_authorization, async (req, res) => {
...
router.delete('/minhareserva', async (req, res) => {
...
router.delete('/reserva', async (req, res) => {
    const parse = ReserveRemove.safeParse(req.query);
    if (!parse.success) {
        return res.status(422).send({
            message: "Dados inválidos",
